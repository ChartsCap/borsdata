...
class BorsdataClient:
    ...
    def branch_breadth(self):
        ...
        stock_prices = pd.DataFrame()
        branch_breadth = pd.DataFrame(columns=['Bransch', '% > MA50'])
        ...
        for branch in my_list:
            branch_instruments = filtered_instruments.loc[(instruments['branch'] == branch)]
            
            for index, instrument in branch_instruments.iterrows():
                # fetching the stock prices for the current instrument
                instrument_stock_price = self._borsdata_api.get_instrument_stock_prices(int(instrument['ins_id']))
                instrument_stock_price.sort_index(inplace=True)
                # using numpy's where function to create a 1 if close > ma50, else a 0
                ...
                stock_prices = pd.concat([stock_prices, df_temp], ignore_index=True)
            try:
                breadth = int(len(stock_prices)/len(branch_instruments) * 100)
            except:
                breadth = None
            branch_breadth = branch_breadth.append({'Bransch': branch, '% > MA50': breadth}, ignore_index=True)
        ...
        #print(branch_breadth.sort_values(by=['breadth'], ascending=False))

        branch_breadth.to_excel('branch-breadth.xlsx', sheet_name='sheet 1', index=False)
    ...
    def sector_breadth(self):
        ...
        sector_breadth.to_excel('sector-breadth.xlsx', sheet_name='sheet 1', index=False)
